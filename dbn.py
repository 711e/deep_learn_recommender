...
from inference import  get_inputX
...
class RBM(object):
    ...
    def rbm_outpt(self, X):
        ...
        out = tf.nn.sigmoid(tf.nn.l2_normalize(tf.matmul(input_X, _w) + _hb,1))
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            return sess.run(out)
...
class NN(object):
    ...
    def __init__(self, sizes, X, Y):
        ...
        self._momentum = 0.0
        self._epoches = 10
        self._batchsize = X.shape[0]
        input_size = X.shape[1]

        # initialization loop
        for size in self._sizes + [Y.shape[1]]:
            # Define upper limit for the uniform distribution range 定义均匀分布范围的上限
            max_range = 4 * math.sqrt(6. / (input_size + size))

            # Initialize weights through a random uniform distribution 通过随机均匀分布初始化权重
            self.w_list.append(
    ...
    def train(self):
        ...
        # Define the training operation (Momentum Optimizer minimizing the Cost function) 定义培训操作（动量优化器最小化成本功能）
...
if __name__ == '__main__':
    ...
    nNet = NN(RBM_hidden_sizes, input_ndarray, target)
